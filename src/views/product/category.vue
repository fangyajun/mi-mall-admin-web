...
export default {
  ...
  methods: {
    ...
    batchDelete() {
      ...
      this.$confirm(`是否批量删除【${ids}】菜单?`, '提示', {
        ...
        .then(() => {
          var param = ids
          console.log(param)
          crudCategory.deleteCategory(param).then(res => {
            this.$message({
              message: '菜单批量删除成功',
              type: 'success'
            })
    ...
    batchSave() {
      var param = this.updateNodes
      crudCategory.updateSort(param).then(res => {
        this.$message({
          message: '菜单顺序等修改成功',
          type: 'success'
        })
    ...
    edit(data) {
      ...
      crudCategory.getNodeInfo(data.id).then(res => {
        // 请求成功
        console.log('要回显的数据', data)
        this.category.categoryName = res.category.categoryName
        this.category.id = res.category.id
        this.category.icon = res.category.icon
        this.category.productUnit = res.category.productUnit
        this.category.parentId = res.category.parentId
        this.category.categoryLevel = res.category.categoryLevel
