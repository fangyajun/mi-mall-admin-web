...
export default {
  ...
  methods: {
    ...
    batchDeleteRelation(val) {
      ...
      crudAttrgroup.deleteAttrgroupRelation(postData).then(data => {
        if (data.code == 0) {
    ...
    relationRemove(attrId) {
      ...
      crudAttrgroup.deleteAttrgroupRelation(data).then(data => {
        if (data.code == 0) {
          this.$message({ type: "success", message: "删除成功" });
          this.init(this.attrGroupId);
        } else {
    ...
    submitAddRealtion() {
      ...
      if (this.innerdataListSelections.length > 0) {
        ...
        crudAttrgroup.addAttrgroupAttrRelation(postData).then(data => {
          if (data.code == 0) {
    ...
    init(id) {
      ...
      this.visible = true;
      crudAttrgroup.getAttrgroupAttrRelation(this.attrGroupId).then(data => {
    ...
    getDataList() {
      ...
      crudAttrgroup.getAttrgroupNoattrRelation(param, this.attrGroupId).then(data => {
        if (data && data.code === 0) {
